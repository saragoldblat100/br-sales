...
/**
 * Generate HTML email content for updated order
 */
const generateOrderUpdateEmailHTML = (order: IOrder): string => {
  const linesHTML = order.lines
    .map(
      (line) => `
      <tr>
  ...
  return `
    ...
    <body>
      ...
      <div class="footer">
        <p>עדכון זה נוצר אוטומטית ממערכת Bravo Sales</p>
        <p>${new Date().toLocaleString('he-IL')}</p>
      </div>
    </body>
    </html>
...
/**
 * Generate plain text email content for updated order
 */
const generateOrderUpdateEmailText = (order: IOrder): string => {
  const lines = order.lines
    .map(
      (line) =>
        `${line.itemCode} | ${line.description} | ${line.cartons} קרטונים | ${line.quantity} יח' | ${formatPrice(line.pricePerUnit, line.currency)} ליחידה | סה"כ: ${formatPrice(line.totalPrice, line.currency)}`
...
---
עדכון זה נוצר אוטומטית ממערכת Bravo Sales
${new Date().toLocaleString('he-IL')}
  `.trim();
};
 ...
 */
export const sendOrderUpdateEmail = async (order: IOrder): Promise<boolean> => {
  const toEmail = env.ORDER_EMAIL_TO;
  ...
  const subject = `עדכון הזמנה ${order.orderNumber} - ${order.customerName}`;
  const html = generateOrderUpdateEmailHTML(order);
  const text = generateOrderUpdateEmailText(order);

  if (!transporter) {
    logger.info('=== UPDATE EMAIL WOULD BE SENT ===');
    logger.info(`To: ${toEmail}`);
    logger.info(`Subject: ${subject}`);
    logger.info(`Content:\n${text}`);
    logger.info('=== END EMAIL ===');
  ...
  try {
    ...
    logger.info(`Order update email sent successfully for order ${order.orderNumber}`);
    return true;
  } catch (error) {
